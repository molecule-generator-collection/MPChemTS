...
class p_mcts:
    ...
    def TDS_UCT(chem_model, hsm, reward_calculator, comm, conf):
        # comm.barrier()
        rank = comm.Get_rank()
            ...
            if jobq_non_empty:
                ...
                if tag == JobType.SEARCH.value:
                    ...
                    if hsm.search_table(message[0]) is None:
                        node = Tree_Node(state=message[0], reward_calculator=reward_calculator, conf=conf)
    ...
    def TDS_df_UCT(chem_model, hsm, reward_calculator, comm, conf):
        # comm.barrier()
        rank = comm.Get_rank()
        nprocs = comm.Get_size()
        status = MPI.Status()
    ...
    def MP_MCTS(chem_model, hsm, reward_calculator, comm, conf):
        #comm.barrier()
        rank = comm.Get_rank()
        nprocs = comm.Get_size()
        status = MPI.Status()
