...
class p_mcts:
    ...
    def __init__(self, communicator, chem_model, property):
        ...
        root_node = Tree_Node(state=['&'], property=self.property)
        random.seed(3)
            ...
            if jobq_non_empty:
                ...
                if tag == JobType.SEARCH.value:
                    ...
                    if self.hsm.search_table(message[0]) is None:
                        node = Tree_Node(state=message[0], property=self.property)
                        #node.state = message[0]
                        if node.state == ['&']:
                            node.expansion(self.self.chem_model)
                            ...
                            if len(node.state) < node.max_len:
                                score, mol = node.simulation(
                                    self.self.chem_model, node.state, self.rank, gau_id)
                                gau_id += 1
